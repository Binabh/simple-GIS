...
def onclick(event):
    ...
    if len(coords) >= 2:
        ...
        tkMessageBox.showinfo("Distance (Km)","Total Length = %s \n Segment Length = %s"%(str(total_dist),str(segment_dist)))
...
def opening_data():
    filename = tkFileDialog.askopenfilename(initialdir = ".",title = "Select file",filetypes = (("shapefiles","*.shp"),("all files","*.*")))
    if filename is None:
        return
    viewing_map(filename)
    

def show_projection_info():
    tkMessageBox.showinfo("CRS info",df.crs['init'])
